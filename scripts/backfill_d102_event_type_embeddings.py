"""Backfill passage embeddings for the D102 travel EventType test nodes.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D102 여행 EventType 노드의 passage embedding을 저장합니다.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
...
def _load_targets() -> list[dict]:
    records, _, _ = neo4j_client.driver.execute_query(
        ...
        ORDER BY node.code
        """,
        codes=list(TARGET_EVENT_TYPE_CODES),
        **_database_kwargs(),
    )

    targets = [dict(record) for record in records]
    found_codes = {target["code"] for target in targets}
    missing_codes = set(TARGET_EVENT_TYPE_CODES) - found_codes

    if missing_codes:
        raise RuntimeError(
            "Neo4j에 대상 EventType이 없습니다: "
