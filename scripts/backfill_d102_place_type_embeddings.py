"""Backfill missing passage embeddings for active PlaceType nodes.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="임베딩이 없는 활성 PlaceType 노드를 배치로 저장합니다.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
...
def main() -> None:
    ...
    try:
        ...
        if not targets:
            print("complete: 임베딩이 필요한 활성 PlaceType이 없습니다.")
            return
