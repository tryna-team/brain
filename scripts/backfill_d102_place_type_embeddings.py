"""Backfill passage embeddings for the D102 travel PlaceType test nodes.
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="D102 여행 PlaceType 노드의 passage embedding을 저장합니다.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
...
def _load_targets() -> list[dict]:
    records, _, _ = neo4j_client.driver.execute_query(
        ...
        ORDER BY node.code
        """,
        codes=list(TARGET_PLACE_TYPE_CODES),
        **_database_kwargs(),
    )

    targets = [dict(record) for record in records]
    found_codes = {target["code"] for target in targets}
    missing_codes = set(TARGET_PLACE_TYPE_CODES) - found_codes

    if missing_codes:
        raise RuntimeError(
            "Neo4j에 대상 PlaceType이 없습니다: "
...
def _generate_embeddings(targets: list[dict]) -> list[dict]:
    ...
    for target in targets:
        ...
        print(f"generated: PlaceType/{code}, dimension={len(embedding)}")
...
def _write_embeddings(transaction, items: list[dict]) -> list[dict]:
    result = transaction.run(
        ...
        MATCH (node:PlaceType {code: item.code})
...
def main() -> None:
    ...
    try:
        ...
        for target in targets:
            print(
                f"validated: PlaceType/{target['code']}, "
