from unittest.mock import Mock

from scripts import backfill_d102_context_embeddings as backfill


def test_load_targets_finds_all_active_contexts_without_embeddings(monkeypatch):
    driver = Mock()
    driver.execute_query.side_effect = [
        ([], None, None),
        (
            [
                {
                    "code": "new_context",
                    "embeddingText": "새로운 맥락",
                    "isActive": True,
                }
            ],
            None,
            None,
        ),
    ]
    monkeypatch.setattr(backfill.neo4j_client, "_driver", driver)

    targets = backfill._load_targets()

    assert targets == [
        {
            "code": "new_context",
            "embeddingText": "새로운 맥락",
            "isActive": True,
        }
    ]
    target_query = driver.execute_query.call_args_list[1].args[0]
    assert "node.isActive = true" in target_query
    assert "node.embedding IS NULL" in target_query
    assert "node.code IN" not in target_query
    assert "node.embeddingText IS NOT NULL" in target_query
    assert "trim(node.embeddingText) <> ''" in target_query


def test_load_targets_reports_contexts_with_empty_embedding_text(
    monkeypatch,
    capsys,
):
    driver = Mock()
    driver.execute_query.side_effect = [
        ([{"code": "empty_context"}], None, None),
        ([], None, None),
    ]
    monkeypatch.setattr(backfill.neo4j_client, "_driver", driver)

    targets = backfill._load_targets()

    assert targets == []
    assert (
        "skipped: Context/empty_context, reason=empty embeddingText"
        in capsys.readouterr().out
    )
    skipped_query = driver.execute_query.call_args_list[0].args[0]
    assert "node.embeddingText IS NULL" in skipped_query
    assert "trim(node.embeddingText) = ''" in skipped_query
